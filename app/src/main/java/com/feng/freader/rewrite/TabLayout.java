...
package com.feng.freader.rewrite;
        ...
        final void update() {
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    ...
                    Log.d(TAG, "update: addView(iconView, 0)");
                    addView(iconView, 0);
                    mIconView = iconView;
                }
                if (mTextView == null) {
...
//                            .inflate(R.layout.design_layout_tab_text, this, false);
    ...
    public class SlidingTabStrip extends LinearLayout {
        ...
        protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
            Log.d(TAG, "onMeasure: run");

            if (MeasureSpec.getMode(widthMeasureSpec) != MeasureSpec.EXACTLY) {
                Log.d(TAG, "onMeasure: run 2");
                ...
                if (largestTabWidth * count <= getMeasuredWidth() - gutter * 2) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        if (lp.width != largestTabWidth || lp.weight != 0) {
                            Log.d(TAG, "onMeasure: largestTabWidth = " + largestTabWidth);
...
//                left = mIndicatorLeft + (mIndicatorRight - mIndicatorLeft) * 0.35f;
//                right = mIndicatorRight - (mIndicatorRight - mIndicatorLeft) * 0.35f;
//
...
//            if (left == 0 && right == 0) {
//                left = mIndicatorLeft + (mIndicatorRight - mIndicatorLeft) * 0.3f;
//                right = mIndicatorRight - (mIndicatorRight - mIndicatorLeft) * 0.3f;
//            }
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        /**
         * @param position 当前显示的第一页的索引（右侧页面进入时显示的是当前页面，左侧页面进入时显示的是左侧页面）
         * @param positionOffset 取值范围 [0, 1)，表示 position 页面的偏移量（在屏幕外的比例）
