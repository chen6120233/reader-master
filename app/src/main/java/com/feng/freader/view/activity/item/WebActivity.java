...
public class WebActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.moshi:
                isMoshi = !isMoshi;
                moshi.setText(isMoshi?"阅读模式":"页面模式");
    ...
    private void setWeb() {
        ...
        settings.setUserAgentString("DNT,X-CustomHeader,Keep-Alive,User-Agent,X-Requested-With,If-Modified-Since,Cache-Control,Content-Type,x-requested-with");
    ...
    String url = "http://m.ltoooo.com/15_15095/9440558.html";

    final class InJavaScriptLocalObj {
        @JavascriptInterface
        public void showSource(String html, String url) {

            if (url.contains("html")||url.contains("wapbook")) {
                String titleName = "";
                String href = "";
                String prev = "";
                ...
                if(TextUtils.isEmpty(data.getPrev_url())&&TextUtils.isEmpty(data.getNext_url())){
                    ...
                    if (isTopActivity(getTopTask(), getPackageName(), WebActivity.class.getName())) {
                        Intent intent = new Intent(WebActivity.this, ReadItemActivity.class);
    ...
    private DetailedChapterData setData(Document document, String titleName, String href, String prev, StringBuffer sb, String url) {
                ...
                if(CustomActivity.URL.getPrev_url().equals(CustomActivity.URL.getNext_url())){
                    ...
                    if (pager != null && pager.size() > 0) {
                        ...
                        LogUtil.v(element.toString());
                        if (element != null && element.size() > 0) {
                            prev = element.get(0).attr("href");
                            if(element.get(2).toString().contains("书签")){
                                href = element.get(3).attr("href");
                            }else {
                    ...
                    LogUtil.v(prev_url.toString());
                    LogUtil.v(next_url.toString());
        ...
        data.setNext_url(href!=null ? href : "");
        data.setUrl(url);
        data.setPrev_url(prev !=null ? prev : "");
